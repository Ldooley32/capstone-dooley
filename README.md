...
1. Combine multiple tables into one.
    - Folders in the file contain multiple tables that need to be combined:
...
3. Removal and Addition of Columns
    - **resistance.csv**:
        a. Remove entries where `agecat` is "All". Combining "Adult" and "Peds" entries while leaving "All" will double the results.  
        b. Add columns: `state_id`, `phenotype_id`, `eventtype_id`, and `agecat_id` to switch variables into integers.
   ...
   - **saar.csv**:
    ...
    b. Add column "saar" calculated as `"observed antimicrobial days" / "predicted antimicrobial days"`.  
    c. Change the state names to abbreviations to match the `State` column in `updated_resistance.csv`.
...
4. **Create a new file `avg_state.csv` to plot the average SAAR value and average percent non-susceptible for each state.**
...
Use the function below to summarize data files:

    ```python
    def summarize_csv(file):
        
        Summarize a CSV file by providing basic information about its structure and content.
